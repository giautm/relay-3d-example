  ...
  return totalCount < 1 ? null : (
    <div className="flex items-center justify-between border-t border-gray-200 bg-white dark:bg-gray-900 px-4 py-3 sm:px-6">
      <div className="flex flex-1 justify-between sm:hidden">
        <Link
          href={getUrl(pathname, searchParams, Math.max(currentPage - 1, 1))}
        ...
        <Link
          href={getUrl(pathname, searchParams, Math.min(currentPage + 1, pages))}
      ...
      <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
        ...
        <div>
          <nav
            ...
            {[...Array(pages)].map((_, i) => (
              <Link
                href={getUrl(pathname, searchParams, i + 1)}
                key={`page-${i}`}
                className={`${
                  i + 1 === currentPage
